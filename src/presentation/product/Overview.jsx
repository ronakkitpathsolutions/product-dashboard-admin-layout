...
import OverviewSlider from '../../common/Sliders/OverviewSlider'
...
import Product3 from './ProductOverview'

const Overview = ({ ...props }) => {
  const { socials, availableColors, id } = useOverview()
  const { wishlists, addToWishList } = useProducts()
  const { data, isLoading, error } = useFetch(`/all-products/get/${id}`)
  ...
  return ( isLoading ? <ProductLoading {...{ loading }} /> :
    ...
    //       <div className='2xs:w-full flex flex-col mx-auto sm:w-4/5 md:w-3/5 lg:w-1/2 lg:h-auto' >
    //         <img alt="ecommerce" className=" mx-auto h-[450px] object-cover object-center rounded" src={data?.data?.product_image} />
    //         <OverviewSlider className="mt-2" sliderData={data?.data?.products_images} />
    //       </div>
